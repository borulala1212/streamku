...
Original file is located at
    https://colab.research.google.com/drive/1Zd3WVX4G6KJtvYYLoMjwpi1mU2DAzo1f
"""
...
BCW = pd.read_csv("Breast_Cancer_Wisconsin.csv")
BCW
...
"""number of rows in dataframe are : 699 
\
...
BCW = BCW.drop(columns = ['Id number'],axis=1)

BCW.isnull().sum()

"""2.6 Fillna Null value """

BCW.update(BCW.fillna(BCW.mean()))

"""2.7 Rechecking null value"""

BCW.isnull().sum()

"""2.8 Checking Statistical data of dataframe"""

BCW.describe()

"""2.9 cheking unique values in dataframe"""

BCW.nunique()

"""Highest number of unique values are in column

2.10 Checking Class Distribution
"""

BCW["Class"]=BCW["Class"].replace(2,0)
BCW["Class"]=BCW["Class"].replace(4,1)

no_disease = BCW[BCW["Class"]==0]
disease = BCW[BCW["Class"]==1]
...
for i in range(len(columns)):
    ...
    plt.subplot(5,2,i+1)
    sns.distplot(BCW[columns[i]], hist=True, rug=True, color=colours[i])
...
for i in (columns):
    ...
    sns.violinplot(x="Class", y=i,data=BCW, kind="violin", split=True, height=4, aspect=.7,palette=colors_list)
       
    sns.swarmplot(x='Class', y=i,data=BCW, color="k", alpha=0.8)
    ...
    for k in range(i,len(columns)):
        try:
            if i==k:
                continue
            plt.subplot(18,2,j)
            sns.scatterplot(x=BCW[columns[i]],y=BCW[columns[k]],hue="Class",data=BCW)
            j=j+1
        except:
            break
...
for i in range(len(columns)):
    plt.subplot(5,2,j)
    sns.stripplot(x='Class', y=columns[i] , data=BCW)
    j=j+1
...
sns.pairplot(BCW, hue='Class')

"""3.6 Correlation"""

correlation_matrix=BCW.corr().round(2)
correlation_matrix["Class"].sort_values(ascending=False)

# Plot Correlation with Heatmap
plt.figure(figsize=(20,10))
sns.heatmap(BCW.corr(),annot=True)
plt.show()
...
sns.set_style(style='white')
sns.countplot(BCW['Class'])

"""## 4. Data Preprocessing

4.1 Dividing the data into X and Y
"""
...
"""4.2 Using RandomOverSampler to handle class imbalance"""
...
"""import prediction RF"""

# Using Joblib
import joblib
# Save Decision Tree Model
model_file_rf = open("RandomForest_model_BreastCancer.pkl","wb")
joblib.dump(model_RF,model_file_rf)
model_file_rf.close()

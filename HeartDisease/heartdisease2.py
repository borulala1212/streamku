...
Original file is located at
    https://colab.research.google.com/drive/1tHc6nK5PxzCH_BhPuzzrCA1ctjZ1Ky8K
"""
...
colours=['b','c','g','k','m','r','y','b','c','g','k','m','r']
...
"""3.2 Violinplot for outcome vs other attributes"""
...
for i in (columns):
    ...
    sns.violinplot(x="target", y=i,data=data, kind="violin", split=True, height=4, aspect=.7,palette=colors_list)
       
    sns.swarmplot(x='target', y=i,data=data, color="k", alpha=0.8)
    ...
    for k in range(i,len(columns)):
        try:
            if i==k:
                continue
            plt.subplot(22,2,j)
            sns.scatterplot(x=data[columns[i]],y=data[columns[k]],hue="target",data=data)
            j=j+1
        except:
            break
...
for i in range(len(columns)):
    plt.subplot(7,2,j)
    sns.stripplot(x='target', y=columns[i] , data=data)
    j=j+1
...
scaled_columns['target'] = data['target'] 
...
model_file_dt = open("DecisionTree_model_HeartDisease.pkl","wb")
joblib.dump(model_DT,model_file_dt)
model_file_rf.close()
...
import joblib
# Save Decision Tree Model
model_file_rf = open("RandomForest_model_HeartDisease.pkl","wb")
...
import joblib
# Save Decision Tree Model
model_file_SVC = open("SVC_model_BreastCancer.pkl","wb")
joblib.dump(classifier,model_file_SVC)
model_file_SVC.close()